...
| 2 | Subagents | **Full ingest, flat-merge with a total ordering contract.** Every `Action` carries `agent_id`. Both layouts supported: `<session-uuid>/subagents/agent-*.jsonl` (2.1.x, has `sessionId` back-pointer) and legacy sibling `agent-<agentId>.jsonl` (linked via `toolUseResult.agentId`). Sort key is `(timestamp, agent lane [main first, then agent_id lexicographic], source line number)` — deterministic regardless of filesystem enumeration. Cross-agent pairs with equal timestamps are marked **order-uncertain** and excluded from findings that need strict before/after semantics (flip, attribution windows); within one agent, line order is always authoritative. **Implemented 2026-07-21** (`feat/subagent-flat-merge`, T5.0): the `flags.subagent_files_missing` counter replaces the pre-merge `subagents_excluded` counter — it reports subagent transcripts that could not be turned into analyzed actions (missing/unreadable/empty/foreign child file, or over the file-count cap), reading `0` once every spawn's work merged in. Constraints: discovery is **one level deep, no recursion**; at most `MAX_SUBAGENT_FILES = 64` child files per session; 2.1.x ownership is validated by the **session-namespaced directory** (`<uuid>/subagents/`) plus a **lenient `sessionId` check** (a present-but-mismatched `sessionId` is rejected, an absent one is accepted); legacy siblings are linked by `toolUseResult.agentId`. **Open item:** the exact 2.1.x on-disk path derivation (`<dir>/<uuid>/subagents/agent-*.jsonl`) is **unverified against a real 2.1.x session** — the donor's child files were unrecoverable, so the layout is proven only against the synthetic `fixtures/subagent-merge/` tree; if a real session shows a different relation, only `locate::subagents_dir` and that fixture's shape change. |
