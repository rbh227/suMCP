//! The install / uninstall write path, the ONLY place backstory-mcp writes anything
...
//!   copied to `<file>.backstory-bak.<unix-seconds>` first.
...
//!   `~/.claude/backstory/manifest.json`; `uninstall` reads it and removes only
...
//! | Target                                   | What                                   |
//! |------------------------------------------|----------------------------------------|
//! | `~/.claude/backstory/bin/backstory{,-mcp}`       | copies of the two binaries             |
//! | `~/.claude/backstory/hooks/stop-nudge.sh`    | the Stop-hook nudge script             |
//! | `~/.claude/backstory/manifest.json`          | the uninstall ledger                   |
//! | `~/.claude/skills/debrief/SKILL.md`      | the debrief skill                      |
//! | `~/.claude.json` → `mcpServers.backstory`    | registers the MCP server (user scope)  |
...
const SERVER_KEY: &str = "backstory";
...
/// The Stop-hook script, also baked in. It shells out to the *installed* backstory
/// binary (absolute path substituted at write time) so it has no dependency on
/// the repo or on `backstory` being on `PATH`.
const HOOK_TEMPLATE: &str = r#"#!/bin/sh
# backstory-mcp Stop-hook nudge (installed by `backstory install`). Reads the transcript;
...
# "transcript_path". We ask the installed backstory binary how many edits the
...
[ -n "${transcript:-}" ] || exit 0
[ -x "$BACKSTORY" ] || exit 0
edits="$("$BACKSTORY" --file "$transcript" --json 2>/dev/null | sed -n 's/.*"edits"[[:space:]]*:[[:space:]]*\([0-9]*\).*/\1/p' | head -n1)"
[ -n "${edits:-}" ] || exit 0
...
marker="${TMPDIR:-/tmp}/backstory-nudged-${session:-$(basename "$transcript" .jsonl)}"
last=0
...
if [ -n "${session:-}" ]; then
  printf '{"systemMessage":"backstory-mcp: session %s has %s edits, run the debrief skill with session_id=%s to see what actually struggled."}\n' "$session" "$edits" "$session"
else
  printf '{"systemMessage":"backstory-mcp: this session had %s edits, run the debrief skill to see what actually struggled."}\n' "$edits"
fi
exit 0
"#;
...
impl Paths {
    ...
    fn backstory(&self) -> PathBuf {
        self.claude().join("backstory")
    }
    fn bin(&self) -> PathBuf {
        self.backstory().join("bin")
    }
    fn hooks(&self) -> PathBuf {
        self.backstory().join("hooks")
    }
    fn manifest(&self) -> PathBuf {
        self.backstory().join("manifest.json")
    }
    ...
    fn installed_backstory(&self) -> PathBuf {
        self.bin()
            .join(format!("backstory{}", std::env::consts::EXE_SUFFIX))
    }
    fn installed_mcp(&self) -> PathBuf {
        self.bin()
            .join(format!("backstory-mcp{}", std::env::consts::EXE_SUFFIX))
    }
...
fn make_backup(path: &Path) -> io::Result<PathBuf> {
    ...
    let secs = now_secs();
    let mut backup = path.with_file_name(format!("{name}.backstory-bak.{secs}"));
    let mut n = 0u32;
    while backup.exists() {
        n += 1;
        backup = path.with_file_name(format!("{name}.backstory-bak.{secs}.{n}"));
    }
    fs::copy(path, &backup)?;
    Ok(backup)
}
...
impl<'a> Journal<'a> {
    ...
    /// `["mcpServers","backstory"]`), preserving everything else. Missing file →
...
/// object we just left empty (so `mcpServers.backstory` removal doesn't strand an
...
/// The `mcpServers.backstory` value: run the installed MCP binary directly.
...
/// bytes, the OLD entry wins: it remembers the state before backstory-mcp ever touched
        ...
        match hit {
            ...
            Some((i, n)) => {
                ...
                match n {
                    ...
                    // recreated: the old entry still describes the pre-backstory-mcp
...
fn run_install(paths: &Paths, exe_dir: &Path, apply: bool) -> io::Result<Vec<String>> {
    ...
    let steps = |j: &mut Journal| -> io::Result<()> {
        ...
        j.place_file(&paths.installed_backstory(), &backstory_bytes, 0o700)?;
        j.place_file(&paths.installed_mcp(), &mcp_bytes, 0o700)?;

        // 3. The Stop-hook script, with the installed backstory path substituted
...
/// Locate the directory holding the running `backstory` binary (its siblings are the
...
pub fn cmd_install(apply: bool) -> io::Result<()> {
    ...
    if cfg!(windows) {
        println!(
            "backstory install: the MCP server and debrief skill are installed, but the \
...
fn print_plan(verb: &str, apply: bool, lines: &[String]) {
    if apply {
        println!("backstory {verb}: applied {} step(s):", lines.len());
    } else {
        println!("backstory {verb} (dry-run, nothing written). Would do:");
    }
    for line in lines {
        println!("  {line}");
    }
...
mod tests {
    ...
    fn insert_and_remove_nested_roundtrip() {
        ...
        assert_eq!(root["mcpServers"]["backstory"]["command"], "x");

        let removed = remove_nested(&mut root, &["mcpServers".into(), "backstory".into()]);
        assert!(removed);
    ...
    fn remove_nested_keeps_sibling_servers() {
        ...
        assert!(remove_nested(
            &mut root,
            &["mcpServers".into(), "backstory".into()]
        ));
    ...
    fn assert_under_home_blocks_escape() {
        ...
        assert!(assert_under_home(Path::new("/Users/x/.claude/backstory"), home).is_ok());
    ...
    /// A directory with dummy `backstory` / `backstory-mcp` "binaries" to copy. Returns
    ...
    /// for `backstory.exe` while this helper had written `backstory`, so eleven install
    ...
    fn fresh_install_writes_all_targets() {
        ...
        assert!(paths.installed_backstory().exists());
        ...
        assert_eq!(
            cj["mcpServers"]["backstory"]["command"].as_str(),
    ...
    fn install_preserves_preexisting_config() {
        ...
        let has_backup = |dir: &Path| {
            fs::read_dir(dir)
                .unwrap()
                .filter_map(|e| e.ok())
                .any(|e| e.file_name().to_string_lossy().contains(".backstory-bak."))
        };
    ...
    fn reinstall_idempotent_then_uninstall_preserves_user_edits() {
        ...
        assert!(
            cj2["mcpServers"].get("backstory").is_none(),
    ...
    fn preexisting_skill_is_backed_up_and_restored() {
        let home = temp_home();
        // A user (or older backstory-mcp) already has a debrief skill with custom text.
    ...
    fn symlinked_skill_dir_target_is_refused() {
        ...
        assert!(!paths.backstory().exists(), "not rolled back");
    }
    ...
    fn partial_failure_rolls_back() {
        ...
        assert!(
            cj["mcpServers"].get("backstory").is_none(),
    ...
    fn failed_reinstall_missing_binary_preserves_previous_install() {
        // An upgrade attempt whose source dir lacks `backstory-mcp` must fail
        ...
        fs::write(
            broken.path().join(format!("backstory{exe_sfx}")),
            b"#!new backstory\n",
        )
        .unwrap();
        // no backstory-mcp sibling
        assert!(
        ...
        assert_eq!(
            fs::read(paths.installed_backstory()).unwrap(),
            b"#!fake backstory\n",
            "previous backstory binary lost"
        );
    ...
    fn failed_reinstall_midway_preserves_previous_install() {
        ...
        let exe_sfx = std::env::consts::EXE_SUFFIX;
        fs::write(
            exe2.path().join(format!("backstory{exe_sfx}")),
            b"#!new backstory\n",
        )
        .unwrap();
        fs::write(
            exe2.path().join(format!("backstory-mcp{exe_sfx}")),
            b"#!new mcp\n",
        )
        .unwrap();
        assert!(
        ...
        assert_eq!(
            fs::read(paths.installed_backstory()).unwrap(),
            b"#!fake backstory\n",
            "failed reinstall did not restore the previous backstory binary"
        );
        assert_eq!(
    ...
    fn manifest_write_failure_rolls_back() {
        ...
        assert!(
            !paths.installed_backstory().exists(),
    ...
    fn install_refuses_scalar_hooks_value() {
        ...
        assert!(!paths.backstory().exists(), "failed install left artifacts");
    }
    ...
    fn install_then_break_restore() -> (TempDir, TempDir, Paths, PathBuf, PathBuf) {
        ...
        let backup = fs::read_dir(&skill)
            .unwrap()
            .filter_map(|e| e.ok())
            .find(|e| e.file_name().to_string_lossy().contains(".backstory-bak."))
