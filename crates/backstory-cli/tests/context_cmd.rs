//! `backstory context` end to end against a fixture transcript.
...
// (mirroring `backstory_core::git`'s own `fixture_repo` pattern) and a throwaway
// transcript with a controlled `effective_ts`, then run the real `backstory
...
/// Run `backstory context --file <transcript> --range HEAD~1..HEAD` inside
