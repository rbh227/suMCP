...
fn html_output_is_self_contained_and_structured() {
    ...
    let out = Command::new(env!("CARGO_BIN_EXE_backstory"))
        .args(["--file", path.to_str().unwrap(), "--html"])
        .output()
        .expect("run backstory");
    assert!(out.status.success(), "backstory --html exited non-zero");
...
fn work_unit_flag_merges_adjacent_transcripts() {
    ...
    let out = std::process::Command::new(env!("CARGO_BIN_EXE_backstory"))
        .arg("--work-unit")
        .arg(&b)
        .arg("--json")
        .output()
