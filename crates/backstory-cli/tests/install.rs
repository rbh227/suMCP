//! End-to-end: drive the real `backstory` binary's `install`/`uninstall` against a
...
fn dry_run_writes_nothing_then_apply_and_uninstall_roundtrip() {
    ...
    let backstory_dir = home.path().join(".claude/backstory");
    ...
    assert!(!backstory_dir.exists(), "dry-run wrote to disk");
    ...
    assert!(
        backstory_dir.join(format!("bin/backstory{exe}")).exists(),
    ...
    assert!(!backstory_dir.exists(), "uninstall left the backstory tree");
    assert!(
...
fn installed_hook_nudges_with_session_id_and_does_not_spam() {
    ...
    let hook = home.path().join(".claude/backstory/hooks/stop-nudge.sh");
