//! Static, self-contained HTML report (`backstory report --html`).
//!
...
pub fn render_html(s: &Session, ranked: &[FileScore], meta: &SessionMeta) -> String {
    ...
    let _ = write!(
        ...
        "<!DOCTYPE html>\n<html lang=\"en\"><head><meta charset=\"utf-8\">\
         ...
         <title>backstory-mcp report — {id}</title><style>{css}</style></head><body>\
         <div class=\"page\">",
        id = esc(&meta.id),
        css = base_css(),
    );
...
fn header_band(s: &Session, meta: &SessionMeta) -> String {
    ...
    format!(
        "<header class=\"hdr\"><span class=\"brand\">backstory-mcp</span>\
...
fn status_bar(s: &Session, o: &crate::report::Overview) -> String {
    ...
    let items = [
        ...
        format!("backstory-mcp v{}", env!("CARGO_PKG_VERSION")),
    ];
    format!(
