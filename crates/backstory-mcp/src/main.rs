//! `backstory-mcp` — MCP server over stdio (T4.1).
//!
...
//! `backstory-core` stays synchronous and pure.
...
/// `$XDG_CONFIG_HOME/backstory/config.toml`, falling back to `~/.config/…`.
...
/// checked-out repo containing `./backstory/config.toml` silently shadow the
/// real one.
fn config_path_from(xdg: Option<PathBuf>, home: Option<PathBuf>) -> Option<PathBuf> {
    ...
    Some(base.join("backstory").join("config.toml"))
}
...
/// `backstory_core::home_dir` (`$HOME`, then `%USERPROFILE%` on Windows) only
/// when `BACKSTORY_CLAUDE_HOME` is unset; the override still wins outright.
fn claude_home() -> Option<PathBuf> {
    std::env::var_os("BACKSTORY_CLAUDE_HOME")
        .map(PathBuf::from)
        .or_else(|| backstory_core::home_dir().map(|h| h.join(".claude")))
}

/// Whether a stale `~/.config/backstory/config.toml` warning should be printed.
...
/// so `~/.config/backstory/config.toml` is no longer read. A user who wrote one is
...
fn warn_if_stale_config(path: Option<PathBuf>) {
    if should_warn_stale_config(&path) {
        ...
        eprintln!(
            "backstory-mcp: {} is no longer read. It used to set ranking weights; \
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let Some(home) = claude_home() else {
        ...
        eprintln!("backstory-mcp: neither BACKSTORY_CLAUDE_HOME nor HOME is set; exiting");
        std::process::exit(1);
    };
    ...
    let server = server::SumcpServer {
        ...
        project_dir: backstory_core::locate::project_dir(&home, &cwd),
...
mod tests {
    ...
    fn empty_or_relative_xdg_config_home_is_ignored() {
        ...
        let expect = PathBuf::from("/home/u/.config/backstory/config.toml");
        ...
        assert_eq!(
            ...
            Some(xdg_base.join("backstory").join("config.toml"))
        );
