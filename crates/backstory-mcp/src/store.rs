...
//! making up one continuous stretch of work, see `backstory_core::work_unit`)
...
fn subagent_sources_fp(dir_fp: &[(PathBuf, u64, u64, bool)], members: &[PathBuf]) -> SubFp {
    let mut fp: SubFp = dir_fp
        .iter()
        .filter(|(p, _, _, _)| backstory_core::locate::is_agent_jsonl(p))
        .cloned()
        .collect();
    for m in members {
        fp.extend(dir_fingerprint(&backstory_core::locate::subagents_dir(m)));
    }
    fp.sort();
    fp
}
...
mod tests {
    ...
    fn store_merges_subagents_and_reparses_on_sub_change() {
        ...
        let sub_actions = a
            ...
            .actions
            .iter()
            .filter(|x| matches!(x.lane, backstory_core::model::Lane::Sub(_)))
            .count();
