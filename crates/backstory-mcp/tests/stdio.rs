...
//!
//! Spawns the real `backstory-mcp` binary (cargo builds it for us and exposes the
//! path as `CARGO_BIN_EXE_backstory-mcp`), speaks line-delimited JSON-RPC over its
...
impl Rpc {
    /// Spawn the server with `BACKSTORY_CLAUDE_HOME` pointed at `claude_home`
    /// and cwd set to `project_cwd` (how Claude Code launches it).
    fn spawn(claude_home: &Path, project_cwd: &Path) -> Rpc {
        let mut child = Command::new(env!("CARGO_BIN_EXE_backstory-mcp"))
            .env("BACKSTORY_CLAUDE_HOME", claude_home)
...
fn fixture_home(root: &Path) -> (PathBuf, PathBuf) {
    ...
    let project_dir = backstory_core::locate::project_dir(&claude_home, &project_cwd);
    std::fs::create_dir_all(&project_dir).unwrap();
    // CARGO_MANIFEST_DIR = crates/backstory-mcp → repo root is two levels up.
...
fn fixture_home_subagents(root: &Path) -> (PathBuf, PathBuf) {
    ...
    // Repo root is two levels up from this crate (crates/backstory-mcp).
...
fn eight_tools_answer_the_frozen_contract_over_stdio() {
    ...
    assert_eq!(init["serverInfo"]["name"], "backstory");
...
fn forwarded_tool_use_id_verifies_the_calling_session() {
    ...
    writeln!(
        f,
        r#"{{"type":"assistant","message":{{"content":[{{"type":"tool_use","id":"toolu_SMOKE_9f3","name":"mcp__backstory__session_overview","input":{{}}}}]}}}}"#
    )
    .unwrap();
    drop(f);
...
fn unverifiable_caller_fails_closed_with_candidates() {
    ...
    let mut rpc = Rpc::spawn(&home, &cwd);
    rpc.handshake();
    // No session_id, no forwarded id, and no fresh backstory marker in any tail:
