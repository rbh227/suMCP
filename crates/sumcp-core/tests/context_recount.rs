...
/// raw JSON; nothing here calls into `sumcp_core`.
struct NaiveCounts {
    decisions: usize,
    claims: usize,
...
fn assert_agrees(label: &str, raw: &str) {
    ...
    let inc = sumcp_core::context::incomplete(&s);
    assert_eq!(
...
///
/// Run manually with: `cargo test -p sumcp-core --test context_recount -- --ignored --nocapture`
#[test]
#[ignore]
fn extraction_agrees_with_an_independent_naive_count_on_real_transcripts() {
    ...
    for path in &files {
        ...
        let d = sumcp_core::context::decisions(&s).len();
        ...
        let c = sumcp_core::context::claims(&s).len();
        if c != naive.claims {
            disagreements.push(format!(
