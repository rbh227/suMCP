...
| Review burden | T1 | Heuristic | Agent-written lines (summed `write_lines` from Edit/Write) between two consecutive human turns, flagged when they exceed the 200-400 line human code-review band | This is the comprehension layer's anchor and runs unconditionally, including under auto-accept, because that is exactly when nobody else is gating the writes. Framed strictly as risk ("this volume plausibly could not have been reviewed"), never a verdict that the human didn't read it, since the transcript can't see their editor. Spans files, so per-file detail needs a follow-up `evidence(idxs)` call. A "human turn" is decided by the transcript's `origin.kind` field: a harness-injected turn (a task notification) does not reset the window; a turn with no `origin` at all counts as human, keeping the old, wider windowing on transcripts written before the field existed. |
| Large-write-instant-accept | T2 | Heuristic | A single main-lane Edit/Write of 2000+ characters whose tool result came back within 3 seconds | A timestamp delta can't distinguish "read it fast" from "auto-accepted" from "stepped away," so it is suppressed for every **action** that ran under an auto-accept permission mode, rather than reported as a meaningless number (unlike review burden, which is never suppressed). Suppression is per action, not per session: the mode is read from the transcript's `mode` events and changes mid-session, so the stretches that ran under normal mode still produce findings. Main-lane only: a subagent write has no human gating it, so the same write on a subagent lane produces no finding. Never reported as exact. |
...
## The reporting unit: work units

Not a finding, but the scope every finding above is computed over. A **work
unit** is the set of transcripts that make up one continuous stretch of work
in one project directory: transcripts join when one begins within 30 minutes
