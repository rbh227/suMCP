...
**Where I struggled:**
1. **src/scene/SceneManager.ts** (churn 24, rework 9, failure loops 3) — my
   camera-init refactor of L120–160 [102] broke `npm test` twice with the same
...
2. **src/App.tsx** (churn 17, rework 6, re-read 5×) — I re-read this file five
   ...
   rewrote the router block once [210, 236].
3. **src/viewer/ViewerCanvas.tsx** (churn 10, 1 fumble) — I attempted an edit
   before ever reading the file [301]; the harness blocked it.

**Blind spots for you:**
- `scripts/export_views.py` was written once and never re-read or verified by
...
**One takeaway:** all three of SceneManager's test failures trace to the
